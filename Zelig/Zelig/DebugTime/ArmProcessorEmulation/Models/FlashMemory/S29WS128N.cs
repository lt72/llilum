...
namespace Microsoft.Zelig.Emulation.ArmProcessor.FlashMemory
{
    ...
    public class S29WS128N : ARMv4Simulator.MemoryHandler
    {
        ...
        const uint ChipSize      = 16 * 1024 * 1024;

        //--//

        static ushort[] sequence_CFI     = new ushort[] { 0x555, 0x0098 };
        ...
        class BankState
        {
            static ushort[] query_results = new ushort[]
            {
                ...
                0x2D, 0x0003, // Erase Block Region 1 Information
                0x2E, 0x0000, //
                0x2F, 0x0080, //
                0x30, 0x0000, //
                ...
                0x35, 0x0003, // Erase Block Region 3 Information
                0x36, 0x0000, //
                0x37, 0x0080, //
                0x38, 0x0000, //
            };
        ...
        public S29WS128N()
        {
            m_banks = new BankState[16];
            m_mode  = Mode.Normal;

            //
            // Bank 0
            //
            m_banks[0] = new BankState( 0, Sector16KWord, 4, Sector64KWord, 7 );

            //
            // Bank 1-14
            //
            for(int bank = 1; bank <= 14; bank++)
            {
                m_banks[bank] = new BankState( m_banks[bank-1].m_addressEnd, Sector64KWord, 8 );
            }

            //
            // Bank 15
            //
            m_banks[15] = new BankState( m_banks[14].m_addressEnd, Sector64KWord, 7, Sector16KWord, 4 );
                ...
                if(m_mode != Mode.Normal)
                {
                    ...
                    if(FindSector( relativeAddress, out bank, out sectorStart, out sectorEnd ))
                    {
                        ...
                        switch(m_mode)
                        {
                            ...
                            case Mode.Program:
                                ushort lastValueShort = (ushort)base.Read( address, relativeAddress, kind );

                                //
                                // Only update the flash if the new value doesn't include any  0 -> 1 transitions, which can only happens through an erase.
                                //
                                if((~lastValueShort & valueShort) == 0)
                                {
